...
export class SceneRenderer {
  ...
  _ensureShaderPipeline(material, geometry) {
    ...
    // Transparent custom-shader materials get standard alpha blending on their
    // color target (the fragment shader returns a vec4 with the alpha it wants).
    let blend;
    ...
    const pipeline = device.device.createRenderPipeline({
      ...
      fragment: { module, entryPoint: 'fragmentMain', targets: [{ format: this.format, ...(blend ? { blend } : {}) }] },
